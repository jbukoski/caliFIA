...
q1_jsf <- db_PLOT_LB %>%
  ...
  filter(n >= 2)         # Without using n_distinct above, it keeps plots with multiple conditions, which is incorrect
...
q1 <- all_dat %>%
  ...
  group_by(PLOT_FIADB, CONDID) %>%    # Group by additional criteria to better identify unique plots, some plots have 
  mutate(n = n_distinct(DSTRBYR1)) %>%
  filter(n >= 2) %>%
...
pass1 <- prtly_cnfrmd %>%
  mutate(keep = match) %>%
  mutate(keep = ifelse(match == FALSE & DSTRBYR1 < 1960, TRUE, keep))
