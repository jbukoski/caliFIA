...
# Rule 1, If plots have two fia fire yaers, two perimeter fire years, the
# max of the perimeter fire year is not greater than the max of the inventory
...
un_p2 <- uncnfrmd %>%
  filter(!(PLOT_FIADB %in% confirmed$PLOT_FIADB) & !(PLOT_FIADB %in% pltsThatFlippedToSingle)) %>%
  group_by(PLOT_FIADB) %>%
  mutate(diffBtwnFires = abs(max(DSTRBYR1) - min(DSTRBYR1))) %>%
  filter(COUNTYCD %in% c(93, 105))


View(un_p2)

un_p2 %>% group_by(SWHW) %>% summarise(n = n_distinct(PLOT_FIADB))

# 12 more softwood plots and 5 more hardwood plots in the unconfirmed dataset - could use plot cards or
# other checks to make judgement calls on whether they should be included.
...
confirmed %>%
  filter(COUNTYCD %in% c(93, 105)) %>%
  select(PLOT_FIADB, INVYR, DSTRBYR1, SWHW) %>%
